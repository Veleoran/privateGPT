# -*- coding: utf-8 -*-
"""
CR Fibre v4.2 — application Flask (mono-utilisateur, auto-hébergée).
Backend Flask + Jinja2 + SQLite WAL. Mail texte brut (Gmail/Brevo).
Scoring : Seul = 1 pt, Binôme = 0,5 pt. Export Excel mensuel de paie. PWA.
v4.1 : dictée vocale terrain (Web Speech API, fr-FR) + copie du CR.
v4.2 : champ code intervention (clé facturation), format réel terrain (équipe/code/
       statut/signature auto), DPR travaux, Excel 2 tableaux, partage Praxedo. Zéro dépendance ajoutée.
"""
import os
import json
...
STATUTS = ["En cours", "Clos", "Travaux", "Clos/Travaux", "Infructueuse"]
EFFECTIFS = ["Seul", "Binôme"]
POINTS = {"Seul": 1.0, "Binôme": 0.5}
EQUIPES = [e.strip() for e in os.environ.get("EQUIPES", "Patrick/Erwan,Patrick/Tevfik").split(",") if e.strip()]
...
def nouveau(key):
    ...
    if request.method == "POST":
        ...
        common = {
            ...
            "dpr": request.form.get("dpr", "").strip(),
    ...
    return render_template(
        ...
        equipes=EQUIPES, points=POINTS, today=today,
    )
...
def renvoyer(iid):
    ...
    code = row["code"] or ""
    if code:
        subject = f"{code} — {row['statut']}".rstrip(" —") if row["statut"] else code
    else:
