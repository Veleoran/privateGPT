# -*- coding: utf-8 -*-
"""CR Fibre v4.2 — couche SQLite stdlib en mode WAL (mono-écrivain, zéro ORM)."""
import os
import sqlite3
import json
...
CREATE TABLE IF NOT EXISTS interventions (
    ...
    code              TEXT,      -- code intervention Praxedo (ex. C35536166-01)
    ...
    points            REAL,     -- 1.0 | 0.5
    statut            TEXT,
    dpr               TEXT,      -- date prochain RDV (travaux)
    consigne          TEXT,
    fields_json       TEXT,
    corps             TEXT,
...
-- idx_code : créé par _migrate() APRÈS l'ajout éventuel de la colonne (base ancienne)
"""

# Colonnes ajoutées après coup : (nom, définition) — migration idempotente.
_ADD_COLUMNS = (
    ("code", "TEXT"),
    ("dpr", "TEXT"),
)
...
def _migrate(con):
    ...
    EXISTS ne modifie pas une table déjà créée). Idempotent et sans perte."""
    existing = {r["name"] for r in con.execute("PRAGMA table_info(interventions)").fetchall()}
    for name, decl in _ADD_COLUMNS:
        if name not in existing:
            con.execute(f"ALTER TABLE interventions ADD COLUMN {name} {decl}")
    con.execute("CREATE INDEX IF NOT EXISTS idx_code ON interventions(code)")
...
def insert_intervention(d):
    ...
    cur = con.execute(
        """INSERT INTO interventions
           (created_at, date_intervention, code, template_key, template_label, equipe,
            effectif, points, statut, dpr, consigne, fields_json, corps,
            mail_envoye, mail_provider, destinataires)
           VALUES (:created_at, :date_intervention, :code, :template_key, :template_label, :equipe,
            :effectif, :points, :statut, :dpr, :consigne, :fields_json, :corps,
            :mail_envoye, :mail_provider, :destinataires)""",
        d,
    )
